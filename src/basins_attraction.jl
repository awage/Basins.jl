...
function basins_map2D(xg, yg, integ; T=0., kwargs...)
    if T>0
    ...
    return draw_basin!((xg, yg), integ, iter_f!, reinit_f!, get_u; kwargs...)
end
...
"""
function basins_general(grid, integ; dt=1., idxs=1:2, Ncheck = 10)
    ...
    bsn_nfo = draw_basin!(
        grid, integ, iter_f!, reinit!, get_projected_state; kwargs...
    )
...
function _identify_basin_of_cell!(
    ...
    nxt_clr = (n[1]==-1  || isnothing(u_full_state)) ? -1 : bsn_nfo.basin[n]
...
function draw_basin_tree!(xg, yg, integ, iter_f!::Function, reinit_f!::Function, get_u::Function, Ncheck, r_init, r_max)
    ...
    bsn_nfo.Na = (bsn_nfo.current_color-2)÷2
    return bsn_nfo
end

function get_data(bsn::AbstractArray,n)
    return bsn[n]
end
...
function set_data!(bsn::AbstractArray,n,c)
    bsn[n]=c
end
